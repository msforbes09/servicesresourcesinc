				...
				<div class="panel-body text-center">
					<div class="col-lg-12">
						...
						<div class="form-group {{ $errors->has('tag') ? ' has-error' : '' }}">
							...
							<input name="tag" type="text" class="form-control" value="{{ $branch->tag }}">
							{!! helpBlock($errors, 'tag') !!}
						</div>
						<div class="form-group {{ $errors->has('phones') ? ' has-error' : '' }}">
							<label>Branch Phone</label>
							<textarea name="phones" class="form-control" rows="2">{{ $branch->phones }}</textarea>
							{!! helpBlock($errors, 'phones') !!}
						</div>
						<div class="form-group {{ $errors->has('emails') ? ' has-error' : '' }}">
							<label>Branch Email</label>
							<textarea name="emails" class="form-control" rows="2">{{ $branch->emails }}</textarea>
							{!! helpBlock($errors, 'phones') !!}
						</div>
						...
						<div class="form-group {{ $errors->has('src_map') ? ' has-error' : '' }}">
							...
							<textarea name="src_map" class="form-control" rows="4">{{ $branch->src_map }}</textarea>
